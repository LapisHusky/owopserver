  ...
  eval: async function (client, args, argsSplit) {
    switch (argsSplit[0]) {
      ...
      case "check": {
        ...
        let target = await client.server.ips.fetch(argsSplit[1])
        if (!client.destroyed) client.sendString(`IP ${argsSplit[1]} is ${target.isWhitelisted() ? "" : "not "}whitelisted.`)
        return
      }
      default: {
...
commands.set("setpbucket", {
  minRank: 3,
  hidden: false,
  eval: function (client, args, argsSplit) {
    if (argsSplit.length < 3) {
      client.sendString("Set a client's drawing limit. (RATE pixels every PER seconds, no floats allowed)")
      client.sendString("Usage: /setpbucket ID RATE PER")
      return
    }
    let id = parseInt(argsSplit[0])
    let target = client.world.clients.get(id)
    if (!target) {
      client.sendString("Client not found!")
      return
    }
    let amount = parseInt(argsSplit[1])
    if (!(amount >= 0 && amount <= 65535)) {
      client.sendString("Set a client's drawing limit. (RATE pixels every PER seconds, no floats allowed)")
      client.sendString("Usage: /setpbucket ID RATE PER")
      return
    }
    let seconds = parseInt(argsSplit[2])
    ...
    client.sendString(`Set client's place bucket: (${amount}, ${seconds})`)
  }
})
commands.set("doas", {
  minRank: 3,
  ...
  eval: function (client, args, argsSplit) {
    if (argsSplit.length < 2) {
      client.sendString("Execute commands as another user. (without the initial '/')")
      client.sendString("Usage: /doas ID COMMAND")
      return
    }
    let id = parseInt(argsSplit[0])
    let target = client.world.clients.get(id)
    ...
    client.sendString("Running command as user.")
    handleCommand(target, `/${args.substring(args.indexOf(" ") + 1)}`)
  }
})
